...
d.all <- cbind(d.all, GetNames(Name))
...
# car.train.grid <- expand.grid(alpha, lambda)
car.train.control <- trainControl(method = "repeatedcv",
                                  number = 3,
                                  repeats = 3,
                                  summaryFunction = defaultSummary)
car.train <- train(Survived ~ 1 + .,
                   data = data,
                   method = "glmnet",
                   family = "binomial",
                   metric = "Accuracy",
...
out <- cbind("PassengerId" = d.init.predict$PassengerId, "Survived" = as.numeric(test.pred)-1)
write.csv(out, file = "../output/predict.csv", row.names=F)